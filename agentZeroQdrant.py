...
# split the docs into chunks by using this function\
...
resume_splits = preprocess_dataset(raw_docs)
...
if qdrant.collection_exists("testing_docs"):
    print("Collection exists. Deleting...")
    qdrant.delete_collection("testing_docs")
    
test_retriever = create_retriever("testing_docs", resume_splits)

retrieve_profile_info = create_retriever_tool(
    test_retriever,
...
async def chat_with_user(state: AgentState):
    ...
    print("old_messages", old_messages and old_messages[-1] if old_messages else "No previous messages")
    
    # Only append the new user message if the last message is not a ToolMessage
    if not old_messages or not isinstance(old_messages[-1], ToolMessage):
    ...
    #     msg_type = type(msg).__name__  # Gets 'HumanMessage', 'AIMessage', etc.
    #     print(f"\n--- Message {i+1} ---")
    #     print(f"Type: {msg_type}")

    #     if hasattr(msg, "additional_kwargs") and msg.additional_kwargs:
    #         print("Additional Kwargs (e.g. function_call):")
    ...
    if not is_final_response:
        ...
        print("Final response content with a tool call:", response)
        return {"messages": old_messages}

    if is_final_response:
        print("there was no tool call")
        print("Final response content with no tool call:", response)
...
# End point to ask agentZero a question with JSON Response
